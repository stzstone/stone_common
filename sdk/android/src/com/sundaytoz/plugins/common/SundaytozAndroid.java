...
import com.sundaytoz.plugins.common.receivers.NetworkChangeReceiver;
...
public class SundaytozAndroid {
	...
	public class Action {
		...
		/* 딥링크 데이터 요청 */
		public static final String GET_AND_CLEAR_DEEPLINK = "GET_AND_CLEAR_DEEPLINK";
		...
		/* 앱설정 이동 */
		public static final String OPEN_APP_DETAIL = "OPEN_APP_DETAIL";
		...
		/* 예약어 설정 */
		public static final String SET_RESERVED_WORD = "SET_RESERVED_WORD";
	...
	{
		ConnectivityManager cm = (ConnectivityManager) UnityPlayer.currentActivity.getApplicationContext().getSystemService(Context.CONNECTIVITY_SERVICE);
		...
		else
		{
			...
			UnityPlayer.currentActivity.getApplicationContext().registerReceiver(new NetworkChangeReceiver(), intentFilter);
		}
	}

	private void onNetworkChange()
	{
		ENetworkType type = NetworkUtil.getStatus(UnityPlayer.currentActivity);
	...
	private void initialize(final Activity inActivity) {
		...
		inActivity.runOnUiThread(new Runnable() {
			@Override
			public void run() {
				Log.d(TAG, "SundaytozAndroid::initialize()->0331_1");
	...
	public JSONObject getDeviceInfo(Activity inActivity) {
		...
		try {
			...
			final String networkCountry = manager.getNetworkCountryIso();
			final String NS_COUNTRY     = simCountry;
			String localeCountry = "";
			if (Build.VERSION_CODES.N <= Build.VERSION.SDK_INT)
				localeCountry = context.getResources().getConfiguration().getLocales().get(0).getCountry();
			else
				localeCountry = context.getResources().getConfiguration().locale.getCountry();
			String country = NS_COUNTRY;
			if (country.length() <= 0)
				country = localeCountry;
			if (localeCountry.length() <= 0)
				country = "Empty";
			Log.d(TAG, "SimCountry: " + simCountry + ", NetworkCountry: " + networkCountry + ", Country: " + country);
			// make result json
			json.put(Action.Param.device_name, android.os.Build.MODEL.toString());
			json.put(Action.Param.carrier,     (carrierName.length() > 0 ? carrierName : "Empty"));
			...
			json.put(Action.Param.local_country, localeCountry);
			json.put(Action.Param.network_country, NS_COUNTRY );
			json.put(Action.Param.country, country);
			...
			json.put(Action.Param.free_space_mb, DiskUtil.freeSpace(true));
			String version_name = "";
			try {
	...
	protected void process(final Activity inActivity, final JSONObject inParam)
	{
        ...
        else if (actionName.equals(Action.GET_FREE_SPACE_MB))
        {
            ...
            {
                JSONObject json = new JSONObject();
                json.put(Action.Param.size, DiskUtil.freeSpace(true));
        ...
        else if (actionName.equals(Action.OPEN_APP_DETAIL))
        {
        	showAppDetail();
        }
		...
		else if (actionName.equals(Action.GET_AND_CLEAR_DEEPLINK))
		{
			try
			{
				JSONObject json = new JSONObject();
				json.put(Action.Param.deeplink, StorageUtil.getDeeplinkData(inActivity.getApplicationContext()));
				Log.v(TAG, "GET_DEEPLINK_DATA(): " + StorageUtil.getDeeplinkData(inActivity.getApplicationContext()));
				sendSuccess(Action.GET_AND_CLEAR_DEEPLINK, json);

				StorageUtil.saveDeeplinkData(inActivity.getApplicationContext(), "");
			}
			catch(Exception e)
			{
				e.printStackTrace();
				sendError(Action.GET_AND_CLEAR_DEEPLINK, new JSONObject(), e.hashCode(), e.getMessage());;
			}
		...
		else if (actionName.equals(Action.SET_RESERVED_WORD))
		{
			String wordKey = inParam.optString(Action.Param.word_key).toLowerCase();
			String wordValue = inParam.optString(Action.Param.word_value);
			Log.d(TAG, "wordKey : " + wordKey + ", wordValue : " + wordValue);
			StorageUtil.saveReservedWord(inActivity.getApplicationContext(), wordKey, wordValue);

			sendSuccess(Action.SET_RESERVED_WORD, new JSONObject());
		}
