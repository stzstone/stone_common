...
public class Main : MonoBehaviour
{
	...
	private void MakeUI()
	{
		...
		ApiTestEditor.Begin("NativeExtension")
			.SetDataAction("Send Email", () => 
				SundaytozNativeExtension.Instance.SendEmail((isSuccess) => { Debug.Log($"Main::SendEmail()->isSuccess:{isSuccess}");}, "jongwoo.park@sundaytoz.com", "title", "content"))
			.SetDataAction("Show Device Data", () => 
				ShowDeviceData())
			.SetDataAction("InAppUpdate Download", () =>
				SundaytozNativeExtension.Instance.DownloadInAppUpdate(OnResponseSimple))
			.SetDataAction("InAppUpdate Install", () =>
				SundaytozNativeExtension.Instance.InstallInAppUpdate(OnResponseSimple))
			.SetDataAction("Add Local Notification",
				() => SundaytozNativeExtension.Instance.AddLocalNotification((success) =>
				{
						
					ApiTestEditor.Log($"Main::AddLocalNotification()->success:{success}");
				}, 1, 5, "title", "message", 1, 1))
			...
			.SetDataAction("Get PushId", () =>
				SundaytozNativeExtension.Instance.GetPushId(OnResponseWithTextContent))
			.SetDataAction("Get Scheme Data", () =>
				SundaytozNativeExtension.Instance.GetSchemeData(OnResponseWithTextContent))
			.SetDataAction("Clear Scheme Data", () =>
				SundaytozNativeExtension.Instance.ClearSchemeData(OnResponseSimple))			
			.SetDataAction("Get Network Status", () =>
				SundaytozNativeExtension.Instance.GetNetworkStatus(OnResponseWithTextContent))
			.SetDataAction("Start Device Loading", () =>
				SundaytozNativeExtension.Instance.StartDeviceLoading(OnResponseSimple))
			.SetDataAction("Stop Device Loading", () =>
				SundaytozNativeExtension.Instance.StopDeviceLoading(OnResponseSimple))
			.SetDataAction("Init Apple Account", () =>
				SundaytozNativeExtension.Instance.InitAppleAccount(OnResponseInitAppleAccount))
			.SetDataAction("Login Apple Account", () =>
				SundaytozNativeExtension.Instance.LoginAppleAccount(onResponseLoginAppleAccount, "ASAuthorizationScopeFullName|ASAuthorizationScopeEmail"))
			.SetDataAction("Logout Apple Account", () =>
				SundaytozNativeExtension.Instance.LogoutAppleAccount(OnResponseSimple))
			.SetDataAction("Get Language", () =>
				SundaytozNativeExtension.Instance.GetLanguage(OnResponseWithTextContent))
			.SetDataAction("Get Free SpaceMb", () =>
				SundaytozNativeExtension.Instance.GetFreeSpaceMb(OnResponseFreeSpaceMb))
...
#elif UNITY_IOS
			.SetDataAction("Is Installed", () =>
				SundaytozNativeExtension.Instance.IsInstalled(OnResponseIsInstalled, "stzkakaojoydev://"))//디즈니 카카오 데브.
