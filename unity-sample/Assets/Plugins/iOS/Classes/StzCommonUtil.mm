...
-(void) processWithData:(NSDictionary *)data
{
    ...
    else if([COMMON_SEND_EMAIL isEqualToString:data[COMMON_ACTION]])
    {
        NSString *receiverAddress = [data[COMMON_RECEIVER_ADDRESS] stringValue];
        NSString *title = [data[COMMON_TITLE] stringValue];
        NSString *body = [data[COMMON_BODY] stringValue];

        [[StzCommonUtil sharedInstance] sendEmail:receiverAddress title:title body:body];
    }
}
...
-(void)setReservedWord:(NSString*)key value:(NSString*)value groupId:(NSString*)groupId
{
    ...
    NSLog(@"StzCommonUtil::setReservedWord()->groupId:%@", groupId);
...
{
    NSLog(@"StzCommon::sendEmail()->%@, %@, %@, canSend : %@", receiverAddress, title, body, ([MFMailComposeViewController canSendMail] ? @"YES" : @"NO"));
    ...
    if([MFMailComposeViewController canSendMail]) {
        ...
        [mailCont setToRecipients:[NSArray arrayWithObject:receiverAddress]];
        [mailCont setMessageBody:body isHTML:NO];
        
        UIViewController *topController = [UIApplication sharedApplication].keyWindow.rootViewController;
        
        [topController presentViewController:mailCont animated:YES completion:nil];
        [[StzCommonUtil sharedInstance] sendSuccess:COMMON_SEND_EMAIL withParam:@{COMMON_SUCCESS : @true}];
    }
    else
    {
        [[StzCommonUtil sharedInstance] sendError:COMMON_SEND_EMAIL withError:nil withParam:@{COMMON_STATUS_MESSAGE:@"Email is not available.", COMMON_SUCCESS : @false}];
    }
}

- (void)mailComposeController:(MFMailComposeViewController*)controller didFinishWithResult:(MFMailComposeResult)result error:(NSError*)error {
    NSLog(@"StzCommonUtil::mailComposeController()->send finished");
    UIViewController *topController = [UIApplication sharedApplication].keyWindow.rootViewController;
